...
- **Paper:** An accompanying paper detailing the project is available in the `article` folder.
...
## Directory Structure
Within the single main branch of the project, there are simply two different folders: the first concerns the collision (`NewSensorCollision`), the second deals with the round with `Roundtriptime`. 
...
1. Copy the folder and move it into your ROS2 workspace `src` folder (e.g., `<ros2_workspace>/src/NewSensorForCollision`).
